...
class PaBoTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--use_bone_branch', type=util.str2bool, nargs='?', const=True, default=True,
                            help='if False, disable G_bone/Sobel/L_bone branch for ablation')
        parser.add_argument('--use_bone_guidance', type=util.str2bool, nargs='?', const=True, default=True,
                            help='if False, disable bone-guided weighting in path loss')
    ...
    def __init__(self, opt):
        ...
        if self.opt.use_bone_branch:
            self.loss_names.append('G_bone')
            self.visual_names += ['G_Bone', 'Sobel_Bone']
        ...
        if self.isTrain:
            ...
            if self.opt.use_bone_branch:
                self.model_names.append('G_bone')
        else:  # during test time, load G (and G_bone only when enabled)
            self.model_names = ['G']
            if self.opt.use_bone_branch:
                self.model_names.append('G_bone')
        ...
        if self.opt.use_bone_branch:
            ...
            self.netG_bone = networks.define_G(opt.input_nc, 1, opt.ngf, "bone_G", opt.normG, not opt.no_dropout,
                                          ...
                                          opt)
            # Sobel(y)
            self.SobelOperator = SobelOperator().to(self.device)
    ...
    def compute_G_loss(self):
        ...
        if self.opt.use_bone_branch:
            ...
            self.loss_G_bone = torch.tensor(0.0, device=self.device)
    ...
    def compute_path_losses(self):
        ...
        loss_path = 0
        use_bone_guidance = self.opt.use_bone_branch and self.opt.use_bone_guidance
